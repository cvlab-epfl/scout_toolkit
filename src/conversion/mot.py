...
def to_mot(input_dict, output_path) -> None:
    """
    Outputs MOT-style annotation text files, one per camera.

    Format per line:
    <frame>, <id>, <bb_left>, <bb_top>, <bb_width>, <bb_height>, -1, <x>, <y>, <z>
    """
    cam_set = set()
    ...
    for cam in cam_list:
        with open(output_dir / f"{cam}.txt", 'w') as f:
            for frame in input_dict['frames']:
                ...
                for annotation in frame["annotations"]:
                    ...
                    line = (
                        f"{frame_id}, {annotation['track_id']}, {x:.2f}, {y:.2f}, "
...
def main():
    ...
    parser.add_argument('--output', type=str, default='mot', help='Output directory for MOT files')
